...
export default function ServiceGrid({ limit, showTitle = true, categorySlug, forceGrouped }: ServiceGridProps) {
  ...
  // Always start with deployed services (SSR-safe), then override with vertical preview if active
  const deployedServices = useMemo(() => getAllServices(), [])
  ...
  }, [verticalServices, deployedServices])
          ...
          <Link href={`/services/${service.slug}`}>
            <Card hover className="h-full group overflow-hidden">
              <div className="aspect-[16/10] bg-muted-foreground/10 relative">
                ...
                {service.image && (
                  <Image src={service.image} alt={service.name} fill className="object-cover" sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" quality={75} />
                )}
                {service.emergency && (
          ...
          <Link href={`/services/${service.slug}`} className="group block">
            <div className="relative">
              <div className="aspect-[4/3] rounded-xl overflow-hidden bg-muted-foreground/10 relative">
                ...
                {service.image && (
                  <Image src={service.image} alt={service.name} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" quality={75} />
                )}
              </div>
  ...
  return (
    <section className="py-16 md:py-24 bg-muted">
      <Container>
        ...
        {currentGrouping === 'grouped' && !categorySlug && groupedServices.length > 0 ? renderGroupedServices() : renderFlatServices()}
      </Container>
    </section>
  )
}
