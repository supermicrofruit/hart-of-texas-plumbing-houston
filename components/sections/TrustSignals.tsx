...
import { Shield, Award, Star, Clock, ThumbsUp, Users, Wrench, AirVent, Flame, Home, Wind, Sparkles, AlertCircle, Truck, Building, Hammer, CheckCircle, Zap, MessageSquare, Calendar, Phone, MapPin, Heart, BadgeCheck, type LucideIcon } from 'lucide-react'
...
const trustIconMap: Record<string, LucideIcon> = {
  ...
  MessageSquare, Calendar, Phone, MapPin, Heart, BadgeCheck,
}
...
function useSignals() {
  ...
  // Use vertical content.json trustSection items if available (design preview mode)
  ...
  // Use deployed content.json trustSection items
  const deployedTrust = (contentData as any).trustSection
  if (deployedTrust?.items?.length) {
    return deployedTrust.items.map((item: any) => ({
      icon: trustIconMap[item.icon] || Shield,
      title: item.label || item.title,
      description: item.value || item.description,
    }))
      ...
      try {
        ...
        if (saved) {
          ...
          if (parsed.trustStyle) {
            setStyle(normalizeTrustStyle(parsed.trustStyle))
            return
          }
        }
      } catch {}
