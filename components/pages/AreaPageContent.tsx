...
// Cross-links: service×area pages for this area
function AreaServiceCrossLinks({ areaSlug, areaName }: { areaSlug: string; areaName: string }) {
  const features = getDefaultFeatures()
  if (!features.showServiceAreaPages) return null

  const pages = (serviceAreasData as any).pages?.filter((p: any) => p.areaSlug === areaSlug) || []
    ...
    <section className="py-12 md:py-16 bg-background">
      <Container>
        <FadeIn>
          ...
          <p className="text-muted-foreground mb-6">Detailed service pages for your area</p>
        </FadeIn>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {pages.map((page: any, index: number) => {
            const service = (servicesData as any).services?.find((s: any) => s.slug === page.serviceSlug)
            return (
              <FadeInStagger key={page.slug} index={index}>
                <Link
                  href={`/services/${page.serviceSlug}/${areaSlug}`}
                  className="group block p-5 bg-muted rounded-xl hover:bg-primary hover:text-white transition-colors"
                >
                  <h3 className="font-semibold text-foreground group-hover:text-white transition-colors mb-1">
                    {service?.name || page.headline}
                  </h3>
                  <p className="text-sm text-muted-foreground group-hover:text-white/70 transition-colors line-clamp-2">
                    {page.description?.slice(0, 100) || service?.shortDescription}...
                  </p>
                </Link>
              </FadeInStagger>
            )
...
function AreaStandard({ area }: AreaPageContentProps) {
  ...
  return (
    <>
      ...
      {/* Cross-links to service×area pages */}
      <AreaServiceCrossLinks areaSlug={area.slug} areaName={area.name} />
...
function AreaMapFocus({ area }: AreaPageContentProps) {
  ...
  // Google Maps Embed API (free, official)
  const mapEmbedUrl = `https://www.google.com/maps/embed/v1/place?key=AIzaSyCB-6kEhfK0GnsduOzINcWkIP5oB2sOAv0&q=${encodeURIComponent(`${area.name}, ${area.state}`)}`
