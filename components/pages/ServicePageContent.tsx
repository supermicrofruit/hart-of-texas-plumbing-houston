...
import { getDefaultFeatures } from '@/lib/useDesignFeatures'
...
// ─── Cross-linking pSEO sections (optional, feature-flagged) ───

function ServiceCrossLinks({ serviceSlug }: { serviceSlug: string }) {
  const features = getDefaultFeatures()
  const guide = features.showCostGuides
    ? (costGuidesData as any).guides?.find((g: any) => g.serviceSlug === serviceSlug)
    : null
  const areaPages = features.showServiceAreaPages
    ? (serviceAreasData as any).pages?.filter((p: any) => p.serviceSlug === serviceSlug) || []
    : []
  const checklist = features.showChecklists
    ? (() => {
        const now = new Date()
        const month = now.getMonth()
        const season = month < 3 ? 'winter' : month < 6 ? 'spring' : month < 9 ? 'summer' : 'fall'
        return (checklistsData as any).checklists?.find((c: any) => c.season === season) || null
      })()
    : null

  if (!guide && areaPages.length === 0 && !checklist) return null

  return (
    <section className="py-12 md:py-16 bg-background" data-section="cross-links">
      <Container>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {/* Cost Guide Card */}
          {guide && (
            <FadeIn>
              <Link href={`/cost-guides/${guide.slug}`}>
                <Card hover className="h-full group p-6">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 bg-green-100 rounded-lg flex items-center justify-center">
                      <span className="text-green-700 font-bold text-sm">$</span>
                    </div>
                    <Badge variant="green" size="sm">Pricing</Badge>
                  </div>
                  ...
                  {guide.priceRanges?.[0] && (
                    <p className="text-2xl font-bold text-primary mb-2">
                      {guide.priceRanges[0].low} – {guide.priceRanges[0].high}
                    </p>
                  )}
                  <p className="text-sm text-muted-foreground">
                    {guide.priceRanges?.length > 1
                      ? `${guide.priceRanges.length} price categories`
            ...
            <FadeIn delay={0.05}>
              <Card className="h-full p-6">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <Badge variant="blue" size="sm">Areas Served</Badge>
                </div>
                <h3 className="text-lg font-semibold text-foreground mb-3">
                  Available in {areaPages.length} areas
                </h3>
                <div className="flex flex-wrap gap-2">
                  {areaPages.map((page: any) => {
                    const area = (areasData as any).areas?.find((a: any) => a.slug === page.areaSlug)
                    return (
                      <Link
                        key={page.slug}
                        href={`/services/${serviceSlug}/${page.areaSlug}`}
                        className="inline-flex items-center px-3 py-1.5 bg-muted rounded-full text-sm font-medium text-foreground hover:bg-primary hover:text-white transition-colors"
                      >
                        {area?.name || page.areaSlug}
                      </Link>
                    )
                  })}
                </div>
                ...
                <Card hover className="h-full group p-6">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <Badge variant="orange" size="sm">
                      {checklist.season.charAt(0).toUpperCase() + checklist.season.slice(1)} Tip
                    </Badge>
                  </div>
                  <h3 className="text-lg font-semibold text-foreground mb-2 group-hover:text-primary transition-colors">
                    {checklist.headline}
                  ...
                  <p className="text-sm text-muted-foreground">
                    {(checklist.items?.length || 0)} maintenance items
                    <ArrowRight className="h-3 w-3 inline ml-1" />
                  </p>
                </Card>
              </Link>
...
function ServiceStandard({ service }: ServicePageContentProps) {
  ...
  return (
    <>
      ...
      {/* Cross-links: Cost Guide, Service Areas, Checklist */}
      <ServiceCrossLinks serviceSlug={service.slug} />
